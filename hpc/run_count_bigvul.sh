#!/bin/bash
# 文件用途：HPC集群上运行BigVul数据集统计分析的SLURM作业脚本
# 该脚本使用Singularity容器执行sastvd/linevd/count_bigvul.py，用于统计和分析BigVul数据集中的漏洞信息
...
#SBATCH --err="hpc/logs/cbv.out"   # 错误输出文件路径
#SBATCH --output="hpc/logs/cbv.out" # 标准输出文件路径
#SBATCH --job-name="cbv"       # 作业名称：cbv
