#!/bin/bash
# 文件用途：HPC集群上运行研究问题1 (RQ1) 实验的SLURM作业脚本
# 该脚本使用Singularity容器执行sastvd/scripts/rq1.py，用于验证LineVD模型在不同配置下的性能
...
#SBATCH --err="hpc/logs/rq1_%A.info"   # 错误输出文件路径，%A为作业ID
#SBATCH --output="hpc/logs/rq1_%A.info" # 标准输出文件路径，%A为作业ID
#SBATCH --job-name="rq1"       # 作业名称：rq1（研究问题1）
