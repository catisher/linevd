...
def feature_extraction(filepath):
    ...
    for cc in reftype_cc:
        ...
        # 获取类型信息
        type_nodes = cc_nodes[cc_nodes["_label"] == "TYPE"]
        if len(type_nodes) > 0:
            # 取第一个结果
            var_type = type_nodes.name.iloc[0]
        else:
            # 设置默认值
            var_type = "unknown"
        # 处理每个标识符节点
...
class IVDetect(nn.Module):
    ...
    def forward(self, g, dataset, e_weights=[]):
        ...
        batch_pooled = torch.zeros((batch_size, 2), device=self.dev)
        start = 0
        for i in range(batch_size):
            num_nodes = g.batch_num_nodes()[i]
            if num_nodes > 0:
                # 计算当前图的节点特征平均值
                batch_pooled[i] = torch.mean(g.ndata["h"][start:start+num_nodes], dim=0)
            start += num_nodes
        return batch_pooled
