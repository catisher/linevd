...
# ==============================================
# 在这里填写不同GNN类型的检查点文件路径
# 格式：{"gnn_type": "GNN类型名称", "checkpoint_path": "检查点文件路径"}
# 示例：
# checkpoint_paths = [
#     {"gnn_type": "gcn", "checkpoint_path": "/path/to/gcn/checkpoint_000099"},
#     {"gnn_type": "gat", "checkpoint_path": "/path/to/gat/checkpoint_000099"},
#     {"gnn_type": "gatv2", "checkpoint_path": "/path/to/gatv2/checkpoint_000099"},
# ]
checkpoint_paths = [
    # 在此处添加数据
...
print("开始使用实际模型进行 myrq3 实验分析...")

# 检查用户是否填写了检查点路径
if not checkpoint_paths:
    print("错误: 请在代码中填写检查点文件路径")
    print("请编辑文件 sastvd/scripts/generate_myrq3_data.py，在 checkpoint_paths 列表中填写检查点文件路径")
    exit(1)

# 设置输出目录
output_dir = str(svd.outputs_dir() / "myrq3_empirical_results")
os.makedirs(output_dir, exist_ok=True)
print(f"输出目录: {output_dir}")

print(f"找到 {len(checkpoint_paths)} 个GNN类型配置:")
for i, item in enumerate(checkpoint_paths):
    print(f"  {i+1}. {item['gnn_type']}: {item['checkpoint_path']}")

# 处理每个GNN类型
for item in checkpoint_paths:
    gnn_type = item['gnn_type']
    ...
    metrics_file = os.path.join(output_dir, f"myrq3_{gnn_type}_metrics.csv")
    ...
    func_output_file = os.path.join(output_dir, f"myrq3_{gnn_type}_func_results.csv")
    ...
    stmt_output_file = os.path.join(output_dir, f"myrq3_{gnn_type}_stmt_analysis.csv")
...
for item in checkpoint_paths:
    gnn_type = item['gnn_type']
    # 读取函数级结果
    func_file = os.path.join(output_dir, f"myrq3_{gnn_type}_func_results.csv")
    ...
    # 读取语句级结果
    stmt_file = os.path.join(output_dir, f"myrq3_{gnn_type}_stmt_analysis.csv")
...
# 保存对比结果
comparison_file = os.path.join(output_dir, "myrq3_gnn_comparison.csv")
