...
该脚本用于对比 GAT（图注意力网络）和 GCN（图卷积网络）两种不同图神经网络的效果。
"""
...
config = {
    ...
    "embtype": tune.choice(["codebert"]),  # 嵌入类型
    ...
    "gnntype": tune.choice(["gat", "gatv2", "gcn"]),  # 对比不同GNN类型
...
# 设置保存路径
sp = svd.get_dir(svd.processed_dir() / f"raytune_rq4_{samplesz}" / run_id)
