...
config = {
    ...
    "modeltype": tune.grid_search(["gat2layer"]),  # 模型架构
    "gnntype": tune.grid_search(["gat", "gatv2", "gcn"]),  # 对比不同GNN类型
...
# 设置保存路径
sp = svd.get_dir(svd.processed_dir() / f"raytune_myrq4_{samplesz}" / run_id)
